...
They should be valid according to the latest Python version, but also to
the previous Python version up until six months after
the release of the current version.
...
start using them on June 23rd, 2017. Since that date the use
of variable annotations is recommended for stubs, even if the implementation
supports Python versions older that 3.6.
