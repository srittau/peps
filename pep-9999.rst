...
Type stubs may define positional-only arguments as per PEP 570 [#pep570]_.
Positional-only arguments in type stubs behave as described in the PEP.

For example::

    def pos_only_args(x: float, y: float, /) -> float: ...

A type checker should report an error for a call like ``pos_only_args(x=1,
y=2)``.
