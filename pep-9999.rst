...
No::
    ...
    class Foo:  # leave only one empty line above
        x: int
    class MyError(Exception): ...  # leave an empty line after the class

Module Level Dunder Names
-------------------------

Do not include ``__all__`` in type stubs.
...
If a function or method returns an instance of a private class, the
type stub should provide a public, stub-only class::
...
Use ``Text`` in mixed Python 2 and 3 stubs when something accepts both
``str`` and ``unicode``. Avoid using ``Text`` in stubs for Python 2 or
3 only or in branches supporting only one Python version.
...
etc.) for arguments and concrete types (``List``, ``Dict``, etc.) for
return values.
